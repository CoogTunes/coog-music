...
class playListAddManager {
  ...
  playListPreviewItem(audioID) {
    ...
    let searchImg = searchItem
      .querySelector(".search-item-img")
      .cloneNode(true);
    let searchInfo = searchItem
      .querySelector(".search-item-info")
      .cloneNode(true);
                                ...
                                <img src="/static/img/abstract-1.jpg">
                                   <div class="playlist-view-wrapper">
                                        <div class="playlist-view-info">
                                           ...
                                           <div class="main-playlist-control control-wrapper" data-playlist-id="{{playlistID}}">
                                            <i class="bi bi-three-dots main-playlist-control-trigger"></i>
                                            <div class="control-playlist-container">
                                            <div class="control-playlist-item delete-playlist table-view">Delete playlist</div>
                                            </div>
                                           </div>                 
                                        </div>
                                   </div>
...
function playListControl() {
  ...
  document.addEventListener("click", function (evt) {
    ...
    if (target.matches(".control-item.add")) {
      ...
      target.parentElement.classList.contains("playlist-view-trigger")
    ) {
      ...
      loadPlaylistIntoView(
        playlistID,
        targetElement.getAttribute("data-view-name")
      );
    }
  });
  ...
  function loadPlaylistIntoView(playlistID, viewName) {
    ...
    ajaxGetHandler("/loadPlaylist?" + data)
      .then((data) => {
        console.log(data);
        updateViewPlaylist(
          data,
